...
class TokenSplitExecutor(
    ...
    private suspend fun executeSingleTokenSplit(
        ...
        // Log the amounts to verify correctness
        val recipientAmount = recipientTokenBeforeTransfer.getCoins().map { it.coins[coinId] }.orElse(null)
        val senderAmount = senderToken.getCoins().map { it.coins[coinId] }.orElse(null)
